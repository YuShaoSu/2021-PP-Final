...
void Image::apply_to_pixel(const int x, const int y)
{
...
{
	// GaussianFilterWorker(0);
	std::vector<std::thread> threads;
	for(int i = 0; i < t_num; ++i) {
		threads.push_back(thread(&Image::GaussianFilterWorker, this, i));
	}
